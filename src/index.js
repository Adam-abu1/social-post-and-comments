...
async function compileData() {
    console.time('Retrieving users from random time delay took');
    let users = await getUsers().then(response => {
        console.timeLog('Retrieving users from random time delay took');
        return response.data;
    });

    console.time('Retrieving post from random time delay took');
    let posts = await getPosts().then(response => {
        console.timeLog('Retrieving post from random time delay took');
        return response.data;
    });

    console.time('Retrieving comments from random time delay took');
    let comments = await getComments().then(response => {
        console.timeLog('Retrieving comments from random time delay took');
        return response.data;
    });
...
/**
 * Reads through the comments against post, if any, and builds out comment elements underneath their respective post
 *
 * @param post
 * @return {Promise<void>}
 */
